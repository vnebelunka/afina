...
void ServerImpl::Join() {
    {
        std::unique_lock<std::mutex> lock(_lock_serv);
        while(running.load() ||  !socket_set.empty()){
...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        {
            std::unique_lock<std::mutex> lock(_lock_serv);
            if(socket_set.size() < _n_workers){
                socket_set.insert(client_socket);
                std::thread client_thread(&ServerImpl::Worker, this, client_socket);
